...
Lê TB_CAP e TB_DIVERSIFICADOR dos arquivos Excel locais e envia para
...
def push_tb_diversificador(conn, dry_run: bool = False) -> int:
    if not config.ARQUIVO_1:
    ...
    # A TB_DIVERSIFICADOR é detalhada por produto: agrega NET por assessor/núcleo
    # antes de enviar ao PostgreSQL, mantendo a tabela enxuta.
    from etl_bases import _agregar_custodia
    df = _agregar_custodia(df, nome_base='TB_DIVERSIFICADOR-PG')

    col_map = {
        config.CUST_ASSESSOR: 'assessor',
        config.CUST_TIME:     'nucleo',
        config.CUST_VALOR:    'net',
    }
    ...
    df_sel = df_sel.where(pd.notnull(df_sel), None)
    rows = [tuple(r) for r in df_sel.values]
    logger.info(f"  → {len(rows)} linhas para captacao.tb_diversificador")

    if dry_run:
        logger.info("  [DRY-RUN] Sem escrita no banco.")
        return len(rows)

    total = _truncate_insert(conn, 'captacao.tb_diversificador', cols_pg, rows)
    return total
...
def main():
    ...
    try:
        ...
        rows_pos = push_tb_diversificador(conn, dry_run=args.dry_run)
        ...
        logger.info("=" * 60)
        logger.info(f"CONCLUÍDO  |  tb_cap: {rows_cap} linhas  |  tb_diversificador: {rows_pos} linhas")
        logger.info("=" * 60)
